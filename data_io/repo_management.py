...
def _clone_repo(repos_directory: Path, repo_url: str, progress: Progress) -> Path | None:
    ...
    try:
        ...
        if repo_path.exists():
            logging.info(f'\n{repo_name} already exists in {repos_directory}, skipping clone\n')
            return repo_path

        logging.info(f'\nCloning Git Repository {repo_url}')

        Repo.clone_from(repo_url,
                        repo_path,
                        progress=GitProgress(progress, description=repo_name))

        logging.info(f'\nFinished cloning {repo_url}')
...
def _load_repo(repos_directory: Path,
    ...
    if repo_path.exists():
        logging.info(f'\nLoading {util.get_repo_name_from_url_or_path(url_or_path)} from disk')
        return RepositoryWithProgress(str(repo_path), num_workers=1, progress=progress)
    else:
        logging.info(f'\nCloning and loading {util.get_repo_name_from_url_or_path(url_or_path)}')
...
def remove_repos(repo_urls: list[str]) -> None:
    ...
    logging.info(f'\nRemoving {len(repo_urls)} repositories {repo_urls}')
